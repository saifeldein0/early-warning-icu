...
# Create comprehensive hematology models plots directory
if not os.path.exists('comprehensive_hematology_models_plots'):
    os.makedirs('comprehensive_hematology_models_plots')
...
# --- Data Loading ---
INPUT_FILE = 'FINAL_HEMATOLOGYy.csv' # Using the larger dataset file
...
print("\n" + "="*80)
print("USING OPTIMAL PARAMETERS FROM PREVIOUS GRID SEARCH")
print("="*80)

# --- Use Best Parameters Found from Previous Grid Search ---
print("\n📋 Using optimal parameters from previous 24-hour grid search...")

# Best parameters from the grid search output
best_xgb_params = {
    ...
    'max_depth': 13, 
        ...
        'n_estimators': 200, 
        'subsample': 0.9
    # Note: scale_pos_weight is handled separately to avoid duplicate parameter error
}
...
print(f"✅ Logistic Regression parameters: {best_lr_params}")
print(f"✅ Random Forest parameters: {best_rf_params}")
print(f"✅ SVM parameters: {best_svm_params}")
print(f"✅ Decision Tree parameters: {best_dt_params}")
print(f"✅ BiLSTM parameters: {best_bilstm_params}")
...
if not os.path.exists('comprehensive_hematology_models'):
    os.makedirs('comprehensive_hematology_models')
    ...
    if model_name == 'BiLSTM':
        if X_train_bilstm_3d_scaled.shape[1] > 0 and X_train_bilstm_3d_scaled.shape[2] > 0:
            ...
            if run_num == 1:
                ...
                }, f'comprehensive_hematology_models/bilstm_best_model.pth')
                print(f"   💾 Saved BiLSTM model to comprehensive_hematology_models/bilstm_best_model.pth (pth format)")
        ...
        if run_num == 1:
            model_filename = f'comprehensive_hematology_models/{model_name.lower()}_best_model.pkl'
            joblib.dump({
                'model': model,
                'model_params': params,
...
results_df.to_csv("comprehensive_models_performance.csv", index=False)
print(f"💾 Saved results to comprehensive_models_performance.csv")
...
plt.tight_layout()
plt.savefig('comprehensive_hematology_models_plots/roc_curves_ordered.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('comprehensive_hematology_models_plots/precision_recall_curves_ordered.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('comprehensive_hematology_models_plots/calibration_curves_ordered.png', dpi=300, bbox_inches='tight')
plt.close()
...
# dca_plot_df_data = {'y_true': y_test.copy()}
# model_names_for_dca = []

# for model_name in model_order:
#     if model_name in latest_results:
#         clean_model_name = model_name.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
#         dca_plot_df_data[clean_model_name] = latest_results[model_name]
#         model_names_for_dca.append(clean_model_name)

# dca_plot_df = pd.DataFrame(dca_plot_df_data)

# if not dca_plot_df.empty and 'y_true' in dca_plot_df.columns and len(model_names_for_dca) > 0:
#     try:
#         plt.figure(figsize=(12, 8))
#         dca_results = dca(data=dca_plot_df, outcome='y_true', modelnames=model_names_for_dca)
#         plot_graphs(dca_results)
#         plt.title('Decision Curve Analysis - Comprehensive Model Comparison\n(Ordered by Average AUC-ROC)', fontsize=14, pad=20)
#         plt.ylim(-0.05, 0.6)
#         plt.grid(True, alpha=0.3)
#         plt.tight_layout()
#         plt.savefig('comprehensive_hematology_models_plots/decision_curve_analysis_ordered.png', dpi=300, bbox_inches='tight')
#         plt.close()
#     except Exception as e_dca:
#         print(f"   ⚠️ Could not generate DCA plot: {e_dca}")
...
for metric in tqdm(metrics_to_plot, desc="Generating Metric Plots", ncols=100):
    ...
    plt.tight_layout()
    plt.savefig(f'comprehensive_hematology_models_plots/{metric.lower().replace("-", "_")}_comparison_ordered.png', 
...
plt.tight_layout()
plt.savefig('comprehensive_hematology_models_plots/performance_heatmap_ordered.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"\n✅ All comprehensive plots saved to 'comprehensive_hematology_models_plots' directory")
print(f"📊 Total results: {len(all_results)} (optimized: {NUM_RUNS} sklearn + {NUM_RUNS_BILSTM} BiLSTM runs)")
print(f"💾 Results saved to: comprehensive_models_performance.csv")
...
final_summary.to_csv('comprehensive_model_comparison_summary.csv', index=False)
print("💾 Model comparison summary saved to: comprehensive_model_comparison_summary.csv")
...
joblib.dump(preprocessing_objects, 'comprehensive_hematology_models/preprocessing_objects.pkl')
print("💾 Preprocessing objects saved to: comprehensive_hematology_models/preprocessing_objects.pkl")

print("\n" + "="*80)
print("🎉 COMPREHENSIVE ANALYSIS COMPLETED SUCCESSFULLY!")
print("="*80)
print("\n📁 Generated Files and Directories:")
print("  📊 Metrics: comprehensive_models_performance.csv")
print("  📈 Summary: comprehensive_model_comparison_summary.csv")
print("  🤖 Models: comprehensive_hematology_models/ directory")
...
print("  📈 Plots: comprehensive_hematology_models_plots/ directory")
...
print("   📝 Sklearn models: joblib.load('comprehensive_hematology_models/[model_name]_best_model.pkl')")
print("   🧠 BiLSTM model: torch.load('comprehensive_hematology_models/bilstm_best_model.pth')")
print("   ⚙️  Preprocessing: joblib.load('comprehensive_hematology_models/preprocessing_objects.pkl')")
